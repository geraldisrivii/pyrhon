            ...
            if (file_index + 1 == len(programm_dict["Existing_files"]) - count_days):
                ...
                with open(f"{path}/{file_index}.json", encoding='utf-8') as main_file:
                    ...
                    for period_index in range(len(main_dicionary)):
                        ...
                        try:
                            amount_learning_time += int(
                                main_dicionary[period_index]["учебное_время"])
                            mental_work += int(
                                main_dicionary[period_index]["когнитивная_нагрузка"])
                        except:
                            continue
...
def match_act_day(activites_data: list, object: list):
    ...
    for i in range(len(activites_data)):
        for key in activites_data[i]:
            try:
                sub_list.append([key,activites_data[i][key], object[i][key]])
            except:
                sub_list.append([key,activites_data[i][key], 0])
